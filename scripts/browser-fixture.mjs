...
const api = httpServer(async (req, res) => {
  ...
  if (url.pathname === '/auth/v1/token') {
    if (body.email && (body.email !== 'tester@example.com' || body.password !== 'TestOnly!123'))
...
const server = await viteServer({
  define: {
    ...
    'import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY': JSON.stringify('local-test-key'),
  },
  server: { host: '127.0.0.1', port: 5175, strictPort: true },
})
await server.listen()
console.log(
  'Local test app: http://127.0.0.1:5175 — tester@example.com / TestOnly!123 (fixture only)',
)
const stop = async () => {
  api.close()
  await server.close()
